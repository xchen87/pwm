...
_WEEKDAY = re.compile(rf"\b(?:by|on|this|next)? ?({'|'.join(_WEEKDAYS)})\b", re.I)
...
def resolve(text: str, written_on: date) -> date | None:
    ...
    if found := _MONTH_DAY.search(text):
    ...
    if found := _WEEKDAY.search(text):
        ahead = (_WEEKDAYS[found[1].lower()] - written_on.weekday()) % 7
        return written_on + timedelta(days=ahead or 7)
