...
### D75. Consent is a condition of starting sign-in (corrected after review)
The first version recorded consent when the login code was redeemed — after Google's callback had already stored the refresh token and queued the first read. That is the wrong order: processing had begun before agreement. Now `/auth/google/start` requires the current terms version and the age attestation (`PWM_MINIMUM_AGE`, 16: the highest age of digital consent in the EU, and above COPPA's 13). What was accepted, and when, is recorded on the account and exported with it. Bumping `PWM_TERMS_VERSION` stops every account (403 `terms_outdated` on everything except reading the terms, accepting them in-app, exporting, signing out and deleting) until it accepts again; the worker reads nothing for a stale account meanwhile. The local development identity is treated as having consented, so the demo path behaves like a signed-up account.
