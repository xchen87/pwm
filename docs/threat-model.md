...
| Control | Status |
|---|---|
| Source text is delimited and labelled as data in every prompt | designed (`prompts/`) |
| Models that read source text have no tools and return schema-validated JSON only | designed |
| Quote verification drops candidates the source does not literally contain | **built** (`pwm.extraction.quotes`) |
| Pipeline output can never set `review`, trigger an action, or delete anything; `user_stated` is accepted only from user-capture sources | designed; the eval **already fails** any system that violates it |
...
A message "from" the user or a known contact at a near-identical address (in the fixture: `examp1e-mail`). Control: entity resolution matches on exact address; display names never merge identities on their own; the user's own statements are accepted only through the app. Status: designed (Slice 1).
...
Controls: every query is scoped by user ID at the data-access layer, not in handlers; a permission-enforcement suite is part of the eval. Status: designed (Slice 4, when accounts exist).
