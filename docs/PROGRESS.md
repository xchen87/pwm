...
**Real-browser sign-in.** The login secret now lives in the browser, so the web sign-in could no longer be walked with `curl`. Added `scripts/signin_e2e.mjs` (drives Chrome over the DevTools protocol; no dependencies, Node 22 only) and `scripts/spa_server.py` (serves the built app with deep-link fallback). The functional test now has a real Chrome click "Continue with Google", pass through the stand-in for Google, land on `/auth?code=…`, redeem the code with its stored secret, and show the signed-in account in Settings; it checks the secret is removed after use and that the session token never appears in a URL. 132 functional checks. The native path (`expo-web-browser` on a device) is still unrun.

**Verification review of the Slice 4 fixes.** A sixth reviewer was asked only to break the fixes in `c75cece`. 12 findings; two were earlier fixes that did not hold. Dispositions:

| # | Sev | Finding | Disposition |
|---|---|---|---|
| 1 | high | "Disconnect is final" did not hold against a sync job **running** in another worker: the disconnect's delete could not see the worker's uncommitted page, waited on row locks, then removed the connection and left 50 sources and their facts behind — with no way to retry | **Fixed.** Sync, disconnect and rebuild all take the same per-user advisory lock *first*; the disconnect waits, then sees and removes the page. Proven with a test using two real database connections and threads. `requeue_stale` was dead code (a job is only "running" inside its own transaction) and is removed; D63 corrected. |
| 2 | med | Calendar silencing broke once the user had **confirmed** an event: a description edit produced an unreviewed twin, and a cancelled meeting stayed a confirmed fact forever | **Fixed.** An event is one event across its versions: a reviewed row carries forward to the new version at the same time, and rows of a called-off event are marked `cancelled` and left out of every live view, keeping the user's review. Test. |
| 3 | med | Reading HTML-only mail was quadratic on hostile input (350 KB of `<style>` ≈ 47 s, inside the sync transaction) | **Fixed.** Input capped at 200k characters; invisible blocks are removed with a linear scan. Timing test. |
| 4 | med | The HTML path let unseen text become the body: class-hidden elements, unclosed `<style>`, `<title>`/`<noscript>`/`<template>`, off-screen positioning, `>` inside a quoted attribute | **Fixed** for all six (parametrized test). Stated plainly in the code: CSS has more ways to hide text than any filter will catch. |
| 5 | med | **Fail-open default**: with `PWM_ENVIRONMENT` unset the server behaved as local development — dev user without a session, demo mailbox, `exp://` redirects — even with a public URL and Google configured | **Fixed.** The default is `production`. "Local" must be asked for, and is believed only when the public URL is loopback. Tests, demo and verify set it explicitly. |
| 6 | med-low | Binding the login code to the app does not stop a **malicious app registered for `pwm://`** from starting its own sign-in and receiving a code bound to its own secret (RFC 8252 §8.6 app impersonation). D62 overstated it | **Documented, not fixable here.** The remedy is claimed HTTPS redirects (universal links / app links), which need a real domain and store identities: Slice 5. D62 and the threat model corrected. Interception of a flow the real app started, and login CSRF, remain closed; the web app has no such residual. |
| 7 | low-med | The sender-authentication parser could be flipped to "pass" by text a sender controls (quoted strings, nested or unclosed comments) | **Fixed.** Quoted strings and all comments are removed first; any DMARC failure outranks a pass. 5 cases tested. |
| 8 | low | Failed connections were retried on every tick forever; a page token that kept failing restarted the backfill in a tight loop | **Fixed.** Failed connections are retried every six hours; stale-token restarts are capped at three. Tests. |
| 9 | low | `Undecryptable` was a `ValueError` and got swallowed by generic handlers (422 instead of 503) | **Fixed.** |
| 10 | low | Values sealed in the first format (`enc:v1:`) became unreadable; one mistyped old key broke every decrypt | **Fixed.** v1 is read by trying each key and upgraded by `reseal`; invalid old keys are ignored. Test. |
| 11 | low | On a phone the login code is likely delivered twice (auth-session result and deep-link route) | **Fixed.** One shared redemption per code. Not run on a device. |
| 12 | low | Redirects were validated parsed but used raw (`pwm://auth#` put the code in a fragment); long calendar ids lost their version suffix | **Fixed.** The raw string is judged; long ids are hashed, not truncated. |

Recorded, not changed: while the data key is missing a user cannot disconnect a source (it needs a rebuild, which needs the bodies) — an operator fault, surfaced as 503; mail rescued from spam later is not read (only `messageAdded` history is followed).

Could not break: challenge enforcement and constant-time comparison; login CSRF; every redirect-matching trick tried; lowercase-hex digests from `expo-crypto` on all platforms; key rotation; reads under a missing key; draft/spam/trash filtering; the worker retry path; calendar ids.
